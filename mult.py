...
from keras.layers import Dense, Input, Bidirectional, LSTM, Embedding, Dropout, Add, Multiply
...
ARCHITECTURE_FILE = os.path.join(MODEL_PATH, "gru_architecture_attn_mult.json")
WEIGHTS_FILE = os.path.join(MODEL_PATH, "gru_weights_attn_mult.h5")
...
def get_model(maxlen, max_features, lstm_size, rate_drop_lstm, rate_drop_dense, embed_size, embedding_matrix):
    ...
    embedded_sequences = Multiply()([embedding_layer_1, embedding_layer_2])
...
def train():
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(MODEL_PATH, "model_attn_mult.h5"), monitor='val_loss',
